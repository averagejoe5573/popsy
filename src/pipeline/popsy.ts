...
import { formatDuration } from "../system/time";
import { createTimestampFolder } from "../system/files";
import { handleCommandError } from "../system/commands";
import { ParsimonyAnalysis } from "./parsimony";
import { performance } from "perf_hooks";
import { MAX_CONCURRENCY } from "../common/constants";
import { LikelihoodAnalysis } from "./likelihood";
...
export class Popsy {
    ...
    public async execute(): Promise<void> {
        ...
        const likelihoodFolderPath = `${relativeTimestampFolderPath}/likelihood`;

        const parsimony = new ParsimonyAnalysis(this.args, this.log);
        const likelihood = new LikelihoodAnalysis(this.args, this.log)

        try {
            await parsimony.executeParsimonyAnalysis(parsimonyFolderPath);
            await likelihood.executeLikelihoodAnalysis(relativeTimestampFolderPath, likelihoodFolderPath)
        } catch (err) {
